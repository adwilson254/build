...
def rename_disabled_directories():
    car_dirs = [
...
def patch_sconscripts():
    ...
    for script in sconscripts:
        ...
        if "'drm'" not in content:
            if "libs += ['yuv']" in content:
                content = content.replace("libs += ['yuv']", "libs += ['yuv']\n  if arch != \"Darwin\":\n    libs += ['va', 'va-drm', 'drm']")
            elif "zstd', 'yuv'" in content:
                content = content.replace("] + base_libs", "] + base_libs\nif arch != \"Darwin\":\n  replay_libs += ['va', 'va-drm', 'drm']")
                content = content.replace("replay_libs +=", "cabana_libs +=") # Quick hack: replace generic for cabana if replay misses. Actually let's use regex
